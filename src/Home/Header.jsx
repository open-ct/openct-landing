...
const LOGO_URL =require("./static/img/OpenCT-logo.png")
...
class Header extends React.Component {
  ...
  componentDidMount() {
    ...
    const fixedTop = document.getElementById('header').offsetTop;
    window.onscroll = () => {
      let scrollTop = Math.max(document.body.scrollTop, document.documentElement.scrollTop)
      //控制元素块A随鼠标滚动固定在顶部
      if (scrollTop - fixedTop >20) {
  ...
  render() {
    const { menuMode, menuVisible, needFixed } = this.state;
    ...
    return (
      <div id="header" className={`header ${needFixed ? 'whiteNav' : ''}`}>
